...
class TomeRater:
    ...
    def create_novel(self, title, author, isbn):
        ...
        for book in self.books:
            if new_fiction.isbn == book.isbn:
                raise Exception("Novel with isbn:{isbn} already exists!".format(isbn=isbn))
        return new_fiction

    def create_non_fiction(self, title, subject, level, isbn):
        new_non_fiction = Non_Fiction(title, subject, level, isbn)
        for book in self.books:
            if new_non_fiction.isbn == book.isbn:
                raise Exception("NonFiction book with isbn:{isbn} already exists!".format(isbn=isbn))
